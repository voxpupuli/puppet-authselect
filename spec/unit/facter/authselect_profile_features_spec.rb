...
require 'spec_helper'
require 'facter'
require 'facter/authselect_profile_features'

describe :authselect_profile_features, type: :fact do
  subject(:fact) { Facter.fact(:authselect_profile_features) }
  ...
  context 'with authselect, but profile with features' do
    ...
    it { expect(fact.value).to eq(%w[with-pamaccess without-pam-u2f-nouserok]) }
  end
end
